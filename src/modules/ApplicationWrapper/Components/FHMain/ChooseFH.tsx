...
import { type ChooseFHType, type ReturnChosenFHType } from './ChooseFH.types'
import { typeFH as FHType } from 'src/domains'
import { observer } from 'mobx-react-lite'
export const ChooseFH = observer(({ elemScreenFh, screenId, dropHook, elemFH, typeFH }: ChooseFHType) => {
  const styles = {
  ...
  return (
    <>
      {elemScreenFh && 'id' in elemScreenFh && elemScreenFh.id ? (
        <ReturnStyledTypeFh elem={elemScreenFh} type={typeFH} dropHook={dropHook} screenId={screenId} />
      ) : Object.keys(elemFH).length !== 0 && elemFH.id ? (
        <ReturnStyledTypeFh elem={elemFH} type={typeFH} dropHook={dropHook} screenId={''} />
      ) : Dragging.draggingActive ? (
        <AbsoluteWrapperBlock refDrag={dropHook} {...styles}></AbsoluteWrapperBlock>
      ) : (
        <div></div>
      )}
    </>
...
const ReturnStyledTypeFh = observer(({ elem, type, screenId, dropHook }: ReturnChosenFHType) => {

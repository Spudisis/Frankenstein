...
import { type SubModules, type Module, type ChangeOptionsProp, type Modules } from 'src/domains'

import { type NestingLayout } from './Layout.types'
import React from 'react'

export const LayoutList = React.memo(
  ({ target, subModule, changeTarget, nesting = 1, changeModules }: NestingLayout) => {
    const changeModulesIn = (changeModule: any) => {
      if (subModule.modules) {
        const newModules = subModule.modules.map((elem: Module | SubModules | undefined) => {
          if (elem) {
            ...
            if (typeof module !== 'undefined') {
              ...
              return (
                <Details
                  ...
                  parent={subModule.id}
                  last={'modules' in module ? !(subModule.modules && subModule.modules.length > 0) : true}
